module.exports = grammar({
  ...
  conflicts: ($) => [
    ...
    [$._statement],
    [$.return_statement],
    [$._rsd_argument, $._expression], 
    [$._rsd_argument, $._macro_name, $._expression], 
    [$._macro_call_without_parentheses, $._macro_call_with_parentheses],
    [$._macro_name, $._subscript_identifier],
  ],
  ...
  rules: {
    ...
    filename: ($) => choice($.identifier, $.string),

    _word_identifier: ($) => /[a-zA-Zа-яА-Я_][a-zA-Zа-яА-Я0-9_]*/,
    _special_identifier: ($) => seq("{", repeat1(/\S/), "}"),
    identifier: ($) => choice($._word_identifier, $._special_identifier),

    _definition: ($) => choice($.macro_definition, $.class_definition),

    _statement: ($) => seq($._statement_core, optional(";")),
    ...
    _typed_identifier: ($) =>
      seq(field("name", $.identifier), optional($.type_declaration)),
    ...
    _identifier_init_list: ($) =>
      prec.left(commaSep1($._variable_initialization)),
    ...
    record_definition: ($) =>
      seq(
        optional($.attribute),
        choice(caseInsensitive("record"), caseInsensitive("file")),
        $.identifier,
        "(",
        commaSep1($.filename),
        ")",
    ...
    attribute: ($) =>
      choice(caseInsensitive("local"), caseInsensitive("private")),
    ...
    for_arguments: ($) =>
      seq(
        "(",
        field("variable", choice($.variable_definition, $.identifier)),
        optional(
          seq(
            ",",
    ...
    return_statement: ($) => seq(caseInsensitive("return"), optional($._expression)),
    ...
    formatted_argument_list: ($) =>
      seq("(", commaSep(seq($._expression, repeat($._format_specifier))), ")"),
    ...
    rsdcommand_call: ($) =>
      choice(
        seq(caseInsensitive("rsdcommand"), "(", repeat($._rsd_argument), ")"), // Со скобками
        seq(caseInsensitive("rsdcommand"), repeat1($._rsd_argument))           // Без скобок
      ),
    ...
    _macro_call_without_parentheses: ($) => seq(field("name", $._macro_name)),

    _macro_call_with_parentheses: ($) =>
      seq(field("name", $._macro_name), $.argument_list),
    ...
    qualification_prefix: ($) => seq($._prefix_item, "."),

    _subscript_identifier: ($) =>
      seq(repeat($.qualification_prefix), choice($.identifier, $.variable_builtin), repeat1($.subscript)),
    ...
    unary_expression: ($) => prec.left(seq($.unary_operator, $._expression)),
    unary_operator: ($) => choice(caseInsensitive("not"), "-"),
  },
});
